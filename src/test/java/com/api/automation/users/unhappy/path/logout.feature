@regression @users @unhappy-path @logout-user @delete
Feature: To test unhappy path for users/logout
	...
	Scenario: Logout twice
		...
		And headers { Accept: "application/json", x-auth-token: "#(getToken.authToken)" }
		When method delete
		Then status 200
	...
	Scenario: Logout without authorization
		And path 'users/logout'
	...
	Scenario Outline: Logout with invalid authorization
		And path 'users/logout'
